...
public class bluestonetorchActive extends BlockTorch {

    /** Whether the redstone torch is currently active or not. */
    private boolean torchActive = false;

    /** Map of ArrayLists of bluestoneupdateinfo. Key of map is World. */
    ...
    private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        ...
        if (par5)
        {
            list.add(new bluestoneupdateinfo(par2, par3, par4, par1World.getTotalWorldTime()));
        }
        ...
        {
            bluestoneupdateinfo bluestoneupdateinfo = (bluestoneupdateinfo)list.get(i1);

            if (bluestoneupdateinfo.x == par2 && bluestoneupdateinfo.y == par3 && bluestoneupdateinfo.z == par4)
            {
                ++l;
    ...
    protected bluestonetorchActive(int ID, boolean On)
    {
        super(ID);
        this.torchActive = On;
    ...
    public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        while (list != null && !list.isEmpty() && par1World.getTotalWorldTime() - ((bluestoneupdateinfo)list.get(0)).updateTime > 60L)
        {
            list.remove(0);
        }
        ...
        if (this.torchActive)
        {
            if (flag)
            {
				par1World.setBlock(par2, par3, par4, Colorstone.bluestonetorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
        ...
        {
			par1World.setBlock(par2, par3, par4, Colorstone.bluestonetorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
        }
    }
    ...
    {
		return Colorstone.bluestonetorchActive.blockID;
    }
	...
	{
		return par1 == Colorstone.bluestonetorchIdle.blockID;
    }

    @SideOnly(Side.CLIENT)
	public String getTextureFile () {
		return CommonProxy.BluestoneTorchActive;
	}
}
