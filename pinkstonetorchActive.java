...
public class pinkstonetorchActive extends BlockTorch {

    /** Whether the redstone torch is currently active or not. */
    private boolean torchActive = false;

    /** Map of ArrayLists of pinkstoneupdateinfo. Key of map is World. */
    ...
    private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        ...
        if (par5)
        {
            list.add(new pinkstoneupdateinfo(par2, par3, par4, par1World.getTotalWorldTime()));
        }
        ...
        {
            pinkstoneupdateinfo pinkstoneupdateinfo = (pinkstoneupdateinfo)list.get(i1);

            if (pinkstoneupdateinfo.x == par2 && pinkstoneupdateinfo.y == par3 && pinkstoneupdateinfo.z == par4)
            {
                ++l;
    ...
    protected pinkstonetorchActive(int ID, boolean On)
    {
        super(ID);
        this.torchActive = On;
    ...
    public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        while (list != null && !list.isEmpty() && par1World.getTotalWorldTime() - ((pinkstoneupdateinfo)list.get(0)).updateTime > 60L)
        {
            list.remove(0);
        }
        ...
        if (this.torchActive)
        {
            if (flag)
            {
				par1World.setBlock(par2, par3, par4, Colorstone.pinkstonetorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
        ...
        {
			par1World.setBlock(par2, par3, par4, Colorstone.pinkstonetorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
        }
    }
    ...
    {
		return Colorstone.pinkstonetorchActive.blockID;
    }
	...
	{
		return par1 == Colorstone.pinkstonetorchIdle.blockID;
    }

    @SideOnly(Side.CLIENT)
	public String getTextureFile () {
		return CommonProxy.PinkstoneTorchActive;
	}
}
