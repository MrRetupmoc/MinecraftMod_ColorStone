...
public class greenstonetorchActive extends BlockTorch {

    /** Whether the redstone torch is currently active or not. */
    private boolean torchActive = false;

    /** Map of ArrayLists of greenstoneupdateinfo. Key of map is World. */
    ...
    private boolean checkForBurnout(World par1World, int par2, int par3, int par4, boolean par5)
    {
        ...
        if (par5)
        {
            list.add(new greenstoneupdateinfo(par2, par3, par4, par1World.getTotalWorldTime()));
        }
        ...
        {
            greenstoneupdateinfo greenstoneupdateinfo = (greenstoneupdateinfo)list.get(i1);

            if (greenstoneupdateinfo.x == par2 && greenstoneupdateinfo.y == par3 && greenstoneupdateinfo.z == par4)
            {
                ++l;
    ...
    protected greenstonetorchActive(int ID, boolean On)
    {
        super(ID);
        this.torchActive = On;
    ...
    public void updateTick(World par1World, int par2, int par3, int par4, Random par5Random)
    {
        ...
        while (list != null && !list.isEmpty() && par1World.getTotalWorldTime() - ((greenstoneupdateinfo)list.get(0)).updateTime > 60L)
        {
            list.remove(0);
        }
        ...
        if (this.torchActive)
        {
            if (flag)
            {
				par1World.setBlock(par2, par3, par4, Colorstone.greenstonetorchIdle.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
        ...
        {
			par1World.setBlock(par2, par3, par4, Colorstone.greenstonetorchActive.blockID, par1World.getBlockMetadata(par2, par3, par4), 3);
        }
    }
    ...
    {
		return Colorstone.greenstonetorchActive.blockID;
    }
	...
	{
		return par1 == Colorstone.greenstonetorchIdle.blockID;
    }

    @SideOnly(Side.CLIENT)
	public String getTextureFile () {
		return CommonProxy.GreenstoneTorchActive;
	}
}
